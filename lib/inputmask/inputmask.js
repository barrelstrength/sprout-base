        ...
        function generateMask(mask, metadata) {
            if (mask == undefined || mask == "") {
                ...
                if (inputmask.prototype.masksCache[mask] == undefined || nocache === true) {
                    ...
                    masksetDefinition = $.extend(true, {}, inputmask.prototype.masksCache[mask]);
                }
    ...
    function maskScope(actionObj, maskset, opts) {
        ...
        function isValid(pos, c, strict, fromSetValid) { //strict true ~ no correction or autofill
            ...
            function _isValid(position, c, strict, fromSetValid) {
                ...
                $.each(getTests(position), function(ndx, tst) {
                    ...
                    rslt = test.fn != null ? test.fn.test(chrs, getMaskSet(), position, strict, opts) : (c == test["def"] || c == opts.skipOptionalPartCharacter) && test["def"] != "" ? //non mask
                        {
                            c: test["def"],
                            pos: position
                        } : false;
        ...
        function caret(input, begin, end) {
            ...
            if (typeof begin == 'number') {
                ...
                if (!androidchrome && opts.insertMode == false && begin == end) {
                    end++;
        ...
        function clearOptionalTail(buffer) {
            ...
            for (; lmib > rl; lmib--) {
                if (isMask(lmib)) {
                    break;
                } //fixme ismask is not good enough
            }
        ...
        function pasteEvent(e) {
            ...
            if (e.type == "propertychange" && input._valueGet().length <= getMaskLength()) {
                ...
                if (valueBeforeCaret == getBufferTemplate().slice(0, caretPos.begin).join('')) {
                    valueBeforeCaret = "";
                }
                if (valueAfterCaret == getBufferTemplate().slice(caretPos.end).join('')) {
                    valueAfterCaret = "";
                }
