...
class DefaultMailer extends BaseMailer implements CampaignEmailSenderInterface
{
	...
	public function getSettingsHtml(array $settings = array())
	{
		...
		$html = Craft::$app->getView()->renderTemplate('sprout-base/_integrations/mailers/defaultmailer/settings', array(
			'settings' => $settings
		));
	...
	public function getListsHtml($values = array())
	{
		...
		return Craft::$app->getView()->renderTemplate('sprout-email/_integrations/mailers/defaultmailer/lists', array(
			'options' => $options,
			'values'  => $selected,
		));
	}
	...
	protected function prepareRecipients($email, $object, $useMockData)
	{
		...
		if ($useMockData)
		{
			...
			if (!empty($invalidRecipients))
			{
				...
				throw new \Exception(Craft::t('sprout-base', "Recipient email addresses do not validate: <br /> {invalidEmails}", array(
					'invalidEmails' => $invalidEmails
				)));
			}
