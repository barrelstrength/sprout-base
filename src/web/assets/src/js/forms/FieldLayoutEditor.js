...
(function($) {
  ...
  class SproutFormsFieldLayoutEditor {
    ...
    createDefaultField(type, tabId, tabName, el) {
      ...
      const defaultName = $(el).data('defaultname') ? $(el).data('defaultname') : Craft.t('sprout', 'Untitled Field');
    ...
    initFieldOnDrop(defaultField, tabName, el) {
      ...
      if (defaultField !== null && defaultField.hasOwnProperty('id')) {
        ...
        Craft.cp.displayError(Craft.t('sprout', 'Something went wrong when creating the field :('));
    ...
    buildFormPageManagerElements($tabs) {
      ...
      this.$formPageManagerNoTabs = $('<p id="notabs"' + ($tabs.length ? ' class="hidden"' : '') + '>' + Craft.t('sprout', 'You don’t have any tabs yet.') + '</p>');
    ...
    initFormPageManagerAdminTable() {
      ...
      window.sproutforms.formPageManager.adminTable = new Craft.AdminTable({
        ...
        sortable: true,
        reorderAction: 'sprout/forms/reorder-form-tabs',
        reorderSuccessMessage: Craft.t('sprout', 'Items reordered.'),
        reorderFailMessage: Craft.t('sprout', 'Couldn’t reorder items.'),
        deleteAction: 'sprout/forms/delete-form-tab',
        confirmDeleteMessage: Craft.t('sprout', "Are you sure you want to delete this tab, all of it's fields, and all of it's data?"),
    ...
    addFormPage() {
      ...
      self.newTabName = prompt(Craft.t('sprout', 'Page Name'));
      ...
      Craft.postActionRequest('sprout/forms/add-form-tab', data, function(response) {
        self.$revisionSpinner.addClass('hidden');
        self.$revisionStatus.removeClass('invisible');
        ...
        if (response.success) {
          ...
          Craft.cp.displayError(Craft.t('sprout', 'Unable to add page.'));
        }
      });
    }
    ...
    renameFormPage(event) {
      ...
      this.newTabName = prompt(Craft.t('sprout', 'Test'), oldName);
      ...
      Craft.postActionRequest('sprout/forms/rename-form-tab', data, function(response) {
        if (response.success) {
          ...
          self.refreshFieldLayout();
          Craft.cp.displayNotice(Craft.t('sprout', 'Page renamed.'));
        } else {
          Craft.cp.displayError(Craft.t('sprout', 'Unable to rename page.'));
        }
      });
    ...
    refreshFieldLayout() {
      ...
      Craft.postActionRequest('sprout/forms/get-updated-layout-html', data, function(response) {
        if (response.success) {
          let $tabs = $('#tabs');
          if ($tabs.length) {
