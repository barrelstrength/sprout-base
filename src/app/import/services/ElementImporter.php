...
class ElementImporter extends Component
{
    ...
    public function saveElement($rows, Importer $importerClass, $seed = false)
    {
        ...
        if (!empty($dataKeysDiff)) {
            ...
            SproutBase::$app->importUtilities->addError($inputKeysText, $message);
        ...
        if (!empty($fields) && method_exists($importerClass, 'getAllFieldHandles')) {
            ...
            foreach ($fields as $fieldHandle => $fieldValue) {
                if (!in_array($fieldHandle, $elementFieldHandles, false)) {
                    ...
                    SproutBase::error($message);
                    SproutBase::$app->importUtilities->addError($key, $message);
        ...
        if ($model->validate(null, false) && $model->hasErrors() == false) {
            ...
            try {
                try {
                    ...
                    SproutBase::$app->importUtilities->addError('save-importer', $message);
                ...
                SproutBase::$app->importUtilities->addError('save-element-error', $message);

                SproutBase::$app->importUtilities->addError('save-element-error-message', $e->getMessage());
            }
        } else {
            $this->logErrorByModel($model);
        }
    ...
    public function getElementFromImportSettings($elementTypeName, $updateElementSettings, $all = false)
    {
        ...
        try {
            ...
            SproutBase::$app->importUtilities->addError('invalid-model-match', $e->getMessage());
    ...
    {
        $elementIds = [];
        $newElements = SproutBase::$app->importUtilities->getValueByKey('newElements', $relatedSettings);
    ...
    {
        $params = SproutBase::$app->importUtilities->getValueByKey('params', $relatedSettings);
         ...
         * If the new 'params' syntax isn't used, use deprecated matchCriteria values if provided
         */
        $matchBy = SproutBase::$app->importUtilities->getValueByKey('matchBy', $relatedSettings);
        $matchValue = SproutBase::$app->importUtilities->getValueByKey('matchValue', $relatedSettings);
