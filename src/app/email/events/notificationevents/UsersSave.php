...
class UsersSave extends NotificationEvent
{
    ...
    public function validateWhenTriggers()
    {
        ...
        if (!$matchesWhenNew && !$matchesWhenUpdated) {
            $this->addError('event', Craft::t('sprout', 'When user is saved event does not match any scenarios.'));
        }

        // Make sure new entries are new.
        if (($this->whenNew && !$isNewEntry) && !$this->whenUpdated) {
            $this->addError('event', Craft::t('sprout', '"When user is created" is selected but the User is being updated.'));
        }

        // Make sure updated entries are not new
        if (($this->whenUpdated && $isNewEntry) && !$this->whenNew) {
            $this->addError('event', Craft::t('sprout', '"When user is updated" is selected but the User is new.'));
        }
    }
    ...
    public function validateUserGroupIds()
    {
        ...
        if ($this->isAdminUser() === false) {
            $errorMessage = Craft::t('sprout', 'Not admin role');
        }
        ...
        if ($this->userGroupIds !== false && is_array($this->userGroupIds)) {
            if ($this->isValidUserGroupIds($currentUsersUserGroups)) {
                ...
                if (!$inGroup) {
                    $errorMessage = Craft::t('sprout', 'Saved user not in any selected User Group.');
                }
            } else {
                $errorMessage = Craft::t('sprout', 'empty user group');
            }
        } else {
            $errorMessage = Craft::t('sprout', 'No User Group has been selected');
        }
