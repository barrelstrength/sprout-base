...
namespace barrelstrength\sproutbase\app\forms\integrations\sproutemail\events\notificationevents;
...
class SaveEntryEvent extends NotificationEvent
{
    ...
    public function validateWhenTriggers()
    {
        ...
        if (!$matchesWhenNew && !$matchesWhenUpdated) {
            $this->addError('event', Craft::t('sprout', 'When a form entry is saved Event does not match any scenarios.'));
        }
    ...
    public function validateEvent()
    {
        ...
        if (get_class($event->entry) !== Entry::class) {
            $this->addError('event', Craft::t('sprout', 'Event Element does not match barrelstrength\sproutforms\elements\Entry class.'));
        }
    }
    ...
    public function validateCaptchas()
    {
        ...
        if ($entry->hasCaptchaErrors()) {
            $this->addError('event', Craft::t('sprout', 'Submitted entry has captcha errors.'));
        }
    }
    ...
    public function validateFormIds()
    {
        ...
        if (!in_array($elementId, $this->formIds, false)) {
            $this->addError('event', Craft::t('sprout', 'The Form associated with the saved Form Entry Element does not match any selected Forms.'));
        }
    }
