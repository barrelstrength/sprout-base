...
class EntryElementIntegration extends ElementIntegration
{
    ...
    {
        return Craft::t('sprout', 'Entry Element (Craft)');
    }
    ...
    public function submit(): bool
    {
        ...
        if ($entryElement->validate()) {
            ...
            $message = Craft::t('sprout', 'Unable to create Entry via Entry Element Integration');
            $this->addError('global', $message);
            Craft::error($message, __METHOD__);
        } else {
            $errors = json_encode($entryElement->getErrors());
            $message = Craft::t('sprout', 'Element Integration does not validate: '.$this->name.' - Errors: '.$errors);
            Craft::error($message, __METHOD__);
