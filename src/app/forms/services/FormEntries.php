...
use barrelstrength\sproutbase\app\forms\events\OnBeforeSaveEntryEvent;
use barrelstrength\sproutbase\app\forms\events\OnSaveEntryEvent;
use barrelstrength\sproutbase\app\forms\jobs\ResaveEntries;
...
class FormEntries extends Component
{
    ...
    {
        if (isset(SproutBase::$app->forms->activeEntries[$form->handle])) {
            return SproutBase::$app->forms->activeEntries[$form->handle];
        }

        $entry = new EntryElement();
        $entry->formId = $form->id;

        SproutBase::$app->forms->activeEntries[$form->handle] = $entry;
    ...
    public function runPurgeSpamElements($force = false)
    {
        ...
        $ids = EntryElement::find()
            ...
            ->orderBy(['sprout_form_entries.dateCreated' => SORT_DESC])
            ->ids();
