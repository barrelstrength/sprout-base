...
namespace barrelstrength\sproutbase\app\seo\fields;
...
class ElementMetadata extends Field
{
    ...
    public function defineRules(): array
    {
        ...
        if (!$isPro && !($theFirstMetadataField || $theOneMetadataField)) {
            $this->addError('optimizedTitleField', Craft::t('sprout', 'Upgrade to Sprout SEO PRO to manage multiple Metadata fields.'));
        }
        ...
        if ($isRequired) {
            ...
            if ($optimizedTitle === 'manually' &&
                ...
                if ($optimizedTitle === 'manually' && empty($value['optimizedTitle'])) {
                    $element->addError(
                        $this->handle,
                        Craft::t('sprout', 'Meta Title field cannot be blank.')
                    );
                }
                ...
                if ($optimizedDescription === 'manually' && empty($value['optimizedDescription'])) {
                    $element->addError(
                        $this->handle,
                        Craft::t('sprout', 'Meta Description field cannot be blank.')
                    );
                }
            }
        }
    ...
    {
        SproutBase::$app->elementMetadata->resaveElementsIfUsingElementMetadataField($this->id);
