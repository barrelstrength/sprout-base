...
class NotificationEmail extends Element
{
	...
	{
		$attributes = [
			'title'       => array('label' => Craft::t('sprout-email', 'Subject Line')),
			'name'        => array('label' => Craft::t('sprout-email', 'Notification Name')),
			'dateCreated' => array('label' => Craft::t('sprout-email', 'Date Created')),
			'dateUpdated' => array('label' => Craft::t('sprout-email', 'Date Updated')),
			'send'        => array('label' => Craft::t('sprout-email', 'Send')),
			'preview'     => array('label' => Craft::t('sprout-email', 'Preview'), 'icon' => 'view')
		];
	...
	public function getTableAttributeHtml(string $attribute): string
	{
		if ($attribute === 'send')
		{
			return Craft::$app->getView()->renderTemplate('sprout-email/_partials/notifications/prepare-link', array(
				'notification' => $this
			));
		}
		...
		if ($attribute === 'preview')
		{
			...
			{
				$shareUrl = UrlHelper::actionUrl('sprout-email/notification-emails/share-notificationEmail', array(
					'notificationId' => $this->id,
				));
			}

			return Craft::$app->getView()->renderTemplate('sprout-email/_partials/notifications/preview-links', array(
				'email'    => $this,
				'shareUrl' => $shareUrl,
				'type'     => $attribute
			));
	...
	public function route()
	{
		...
		if (!Craft::$app->getView()->doesTemplateExist($this->template . $extension))
		{
			...
			SproutEmail::$app->utilities->addError(Craft::t('sprout-email', "The template '{templateName}' could not be found", array(
