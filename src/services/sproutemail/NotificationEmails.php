...
class NotificationEmails extends Component
{
		...
		if (!empty($notificationEmail->id))
		{
			...
			if (!$notificationEmailRecord)
			{
				throw new \Exception(Craft::t('sprout-email',
					'No entry exists with the ID “{id}”', array('id' => $notificationEmail->id)));
			}
		}
		else
		{
	...
	public function sendTestNotificationEmail(NotificationEmail $notificationEmail)
	{
		try
		{
			...
			return Response::createModalResponse('sprout-email/_modals/response', array(
					'email'      => $notificationEmail,
		...
		catch (\Exception $e)
		{
			...
			return Response::createErrorModalResponse('sprout-email/_modals/response', array(
				'email'   => $notificationEmail,
	 ...
	 * @return array
	 */
	public function getNotificationErrors($notificationEmail, $errors = array())
	{
		...
		if (empty($event))
		{
			$errors[] = Craft::t('sprout-email', 'No Event is selected. <a href="{url}">Edit Notification</a>.', array(
